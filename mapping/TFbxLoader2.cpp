...
// TMatrix는 행백터용 행렬, TFbx는 열백터용 행렬이므로
// 전치과정을 거친다.
...
void TFbxLoader::PreProcess(FbxNode* Node, TFbxModel* fbxParent)
{
	...
	if (Node != nullptr)
	{
		...
		// 트리의 모든 로컬 변환 행렬을 제거
		m_TreeList.push_back(fbx); // 각각의 노드에 자기자신의 로컬행렬정보만 담는다.
	}
...
void TFbxLoader::ParseMesh(TFbxModel* pObject)
{
	...
	// 머테리얼이 하나일댄 노드에서 가지고오나 레이어에서 가지고 오나 같은 정보
