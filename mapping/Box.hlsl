...
{
	//65535 / 4 개만큼의 행렬을 사용가능하다.
	float4x4 g_matBoneWorld[255]; 
}
VS_OUTPUT VS(VS_INPUT v)
{
	VS_OUTPUT pOut = (VS_OUTPUT)0;
	uint  iIndex = v.c.w;
	// 기존엔 프레임에서 애니메이션 행렬을 모두 계산해서 파이프랜더링을 호출했지만
	// 이젠 애니메이션 행렬도 파이프라인에 넘겨서 버택스 셰이더에서 애니메이션 작업시행.
	float4 vLocal = float4(v.p.xyz, 1.0f);// float4(v.p.x, v.p.y, v.p.z, 1.0f);
	float4 vWorld = mul(vLocal, g_matBoneWorld[iIndex]);
	vWorld = mul(g_matWorld, vWorld);
