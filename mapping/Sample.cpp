...
{	
	...
	listname.push_back(L"../../data/fbx/Sphere.FBX");
	...
	m_pTmpTex = I_Texture.Load(L"../../data/pung00.dds");
	m_pLightTex = I_Texture.Load(L"../../data/charport.bmp");
	m_pNormalMap = I_Texture.Load(L"../../data/NormalMap/stone_wall_normal_map.bmp");
	return true;
}
bool	Sample::Frame()
{		
...
{		
	// 여기서 조명으로 사용할 텍스쳐 추가
	m_pImmediateContext->PSSetShaderResources(0, 1, m_pTmpTex->m_pSRV.GetAddressOf());
	m_pImmediateContext->PSSetShaderResources(1, 1, m_pLightTex->m_pSRV.GetAddressOf());
	m_pImmediateContext->PSSetShaderResources(2, 1, m_pNormalMap->m_pSRV.GetAddressOf());
	for (int iObj = 0; iObj < m_FbxObj.size(); iObj++)
	{
