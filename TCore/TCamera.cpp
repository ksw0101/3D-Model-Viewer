...
bool TCamera::Frame()
{
	...
	{
		m_fSpeed = min(m_fSpeed + g_fSecPerFrame * 500.0f, 5000.0f);
	}
	else
	{
		m_fSpeed = max(m_fSpeed - g_fSecPerFrame * 500.0f, 100.0f);
	}
	...
	if (TInput::Get().GetKey('Q'))
	{
		MoveUp(g_fSecPerFrame * m_fSpeed);
	}
	if (TInput::Get().GetKey('E'))
	{
		MoveUp(-g_fSecPerFrame * m_fSpeed);
	}
